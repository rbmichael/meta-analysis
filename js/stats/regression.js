...
export function lsModel(studies, locMods = [], scaleMods = [], opts = {}) {
  ...
  return {
    ...
    k: kf, p, q,
    rankDeficient: false,
    convergence: { converged: opt.converged && !rdGamma, iters: opt.iters, maxIters: 600,
                   reason: !opt.converged ? 'max_iters' : rdGamma ? 'singular_hessian' : null,
                   source: 'bfgs_locScale' },
    studiesUsed: rows,
    fitted, residuals,
    yi, vi,
    labels: rows.map(s => s.label || ""),
