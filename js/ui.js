...
import { fmt, normalQuantile, normalCDF, chiSquareCDF, tCritical } from "./utils.js";
...
function _renderMVResults(res, { alpha, rows = [] } = {}) {
  ...
  alpha ??= 0.05;
  const ciPct  = Math.round((1 - alpha) * 100);
  const fmtP   = p => !isFinite(p) ? "—" : p < 0.0001 ? p.toExponential(2) : fmt(p, 4);
  ...
  if (struct === "CS") {
    ...
    psiBlock = `<h4 style="font-size:0.9em;margin:8px 0 4px">Between-study heterogeneity (Ψ̂, CS)</h4>
      ...
      <table class="reg-table" style="margin-bottom:8px">
        <thead><tr><th>Outcome</th><th>τ²</th><th>I²</th></tr></thead>
        <tbody>${i2rows}</tbody>
      </table>`;
  } else if (struct === "Diag") {
    const i2rows = outcomeIds.map((id, o) =>
...
function _buildMVReportHTML(res, rows = [], alpha = 0.05) {
  ...
  const fmtP  = p => !isFinite(p) ? "—" : p < 0.0001 ? p.toExponential(2) : (+p).toFixed(4);
...
function _renderAllResults(ctx) {
  ...
  const logScaleFELine = showLogScale
    ...
    : "";
  const showFisherZ = !!profile.isFisherZ;
  const fisherZRELine = showFisherZ
    ? `<div class="result-log-note">Fisher z scale: ${fmt(m.RE)} | SE = ${fmt(m.seRE)} | ${ciLbl} [${fmt(m.ciLow)}, ${fmt(m.ciHigh)}]</div>`
    : "";
  const fisherZFELine = showFisherZ
    ? `<div class="result-log-note">Fisher z scale: ${fmt(m.FE)} | SE = ${fmt(m.seFE)} | ${ciLbl} [${fmt(m.FE - feZ * m.seFE)}, ${fmt(m.FE + feZ * m.seFE)}]</div>`
    : "";
  ...
  ` : `
    ...
    ${showFisherZ && isFinite(m.predLow) ? `<div class="result-log-note">Fisher z scale: [${fmt(m.predLow)}, ${fmt(m.predHigh)}]</div>` : ""}
